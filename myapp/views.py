...
class IngredientViewSet(viewsets.ModelViewSet):
    ...
    permission_classes = [IsAdmin]  # Changed to IsAdmin to restrict access
...
def verify_payment_api(request, control_number):
    try:
        ...
        if random.random() > 0.5:  # Mock verification logic
...
def assign_technician_to_sample(request):
    if request.user.role != 'HOD':
        return Response({'success': False, 'message': 'Access denied. HOD role required.'}, status=status.HTTP_403_FORBIDDEN)
    logger.debug(f"Current HOD assigning: {request.user.username}, ID: {request.user.id}")
    ...
    try:
        ...
        # Only set assigned_to_hod if it’s not already set
        if not sample.assigned_to_hod:
